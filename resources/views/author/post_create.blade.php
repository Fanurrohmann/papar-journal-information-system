...
@section('button')
<a href="{{ route('author_home') }}" class="btn btn-primary"><i class="fas fa-eye"></i> View</a>
@endsection
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="editor_id" class="form-control select2" required>
                                ...
                                @foreach($editors as $item)
                                    <option value="{{ $item->id }}" {{ (old('editor_id') == $item->id) || (Auth::guard('editor')->user()->id == $item->id) ? 'selected' : '' }}>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
